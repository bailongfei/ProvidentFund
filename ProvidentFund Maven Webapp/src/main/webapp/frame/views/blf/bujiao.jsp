...
<body>
 ...
 <!-- 模态框（Modal） -->
<div class="modal fade" id="myModalbjmx" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content" style="width: 800px;">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel" >
					单位信息:<input style="text-align:right;"  type="date"    id="date1" name=""  >至<input style="text-align:right;"  type="date"    id="date2" name="" ><button id="btnmx" id='bjid' onclick="" class="huijiaomx btn btn-info btn-xs">检索</button>
				</h4>
			</div>
			<div class="modal-body">
 ...
 <!-- <ul class="pager" >
 	<li>
 	当前页:<span id="curPage1"></span>总页数:<span id="totalPages1"></span>
 	</li>
 	<li><a  id="first1" onclick="changePages(this)">首页</a></li>
 	<li><a  id="prev1" onclick="changePages(this)">上一页</a></li>
 	<li><a  id="next1" onclick="changePages(this)">下一页</a></li>
 	<li><a  id="last1"  onclick="changePages(this)">尾页</a></li>
 	<li><input type="number"  id="txtCurPage1"   min="1" max="10"></input><input type="button" class="btn btn-default" value="go" onclick="gotoPages()"/></li>
  </ul> -->
 	</div>
		...
		$.ajax({
			...
			success : function(data) {
				...
				for (var i = 0; i <ob.length; i++) {
					...
					tr += "<td><input type='button'    data-toggle='modal' data-target='#myModal'  id=" + obj.UnitInfoAccount + "  class='huijiaoSelect btn btn-info' value='补缴办理'><input type='button'    data-toggle='modal' data-target='#myModalbjmx'  id=" + obj.UnitInfoAccount + "  class='huijiaomx btn btn-info' value='补缴明细'></td>";
           ...
           success:function(data){
              ...
              for(var i=0;i<data.length;i++){
                 ...
                 var tr="<tr class='checkcc'>";
					 ...
					 tr+="<td><input type='text'  size='6' value="+parseInt(val.personalDepositBase)*parseFloat(val.unitRateDeposit)+" class='dwyjce' id='' name='dwjn"+i+"' readonly='readonly'></td>";
					 tr+="<td><input type='text'  size='6' value="+parseInt(val.personalDepositBase)*parseFloat(val.perContributionRate)+" class='gryjce' id='' name='grjn"+i+"' readonly='readonly'></td>";
...
/* 明细 */
$(document).on("click",".huijiaomx",function(){
    var UnitInfoAccount=this.id;
    /* var pis=$(".bjmxid").children().eq(0).html();
    alert(pis); */
    var date1=$("#date1").val();
    var date2=$("#date2").val();
    $.ajax({
	       url:"${pageContext.request.contextPath}/ipay/querybjmx",
	       type:"post",
	       data:{"UnitInfoAccount":UnitInfoAccount,"date1":date1,"date2":date2},
	       dataType:"json",
	       success:function(data){
	         ...
	         $.each(data,function(key,val){
	            ...
	            tr+="<td>"+val.personalAccount+"</td>";
	            tr+="<td>"+val.bkname+"</td>";
	            /* tr+="<td>"+val.demandInterestAnd+"</td>"; */
	            tr+="<td>"+val.unitAdvancep+"</td>";
	            tr+="<td>"+val.personAdvancep+"</td>";
	            tr+="<td>"+val.perbuLimit+"</td>";
	            tr+="<td>"+(parseFloat(val.unitAdvancep)+parseFloat(val.personAdvancep))+"</td>";
	            tr+="<td>"+val.grperbuTime+"</td>";
	            tr+="</tr>";
	            $("#tbody3").append(tr);
	         });
	        // alert(data[0].UnitInfoAccount);
	         $("#bjid").attr("id",data[0].UnitInfoAccount);
	           }
	     });
});
/* function hjmxcx(page){
        var UnitInfoAccount=$("#ids").val();
        var date1=$("#date1").val();
        var date2=$("#date2").val();
	    $.ajax({
	       url:"${pageContext.request.contextPath}/hjqcb/selectHjmx",
	       type:"post",
	       data:{"UnitInfoAccount":UnitInfoAccount,"date1":date1,"date2":date2,"pageNum":page},
	       dataType:"json",
	       success:function(data){
	         $("#tbody3").empty();
	         var ob=data.list;
	         ...
	         //重新初始化分页链接
        $("#curPage1").html(data.curPage);
		$("#totalPages1").html(data.totalPages);
        $("#first1").attr("data",data.first);
		$("#prev1").attr("data",data.prev);
		$("#next1").attr("data",data.next);
		$("#last1").attr("data",data.last);
		$("#txtCurPage1").val(data.curPage).attr("max",data.last);
